...
function service($what = null)
{
    ...
    if (!$di) {
        ...
        $di = $builder->newConfiguredInstance([
            \Ushahidi\Modules\V3\ContainerConfig\CoreConfig::class,
            \Ushahidi\Modules\V3\ContainerConfig\AppConfig::class,
            \Ushahidi\Modules\V3\ContainerConfig\LaravelConfig::class,
        ]);
    }
    if ($what) {
        return $di->get($what);
