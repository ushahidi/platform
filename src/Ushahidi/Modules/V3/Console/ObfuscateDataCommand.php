...
class ObfuscateDataCommand extends Command
{
    ...
    */
    ...
    protected function overwriteContacts()
    {
        ...
        $resultPhoneCount = DB::table('contacts')
            ->where('type', 'phone')
            ->update(['contact' => $randomPhoneGenerator]);

        $resultTwitterCount = DB::table('contacts')
            ->where('type', 'twitter')
            ->update(['contact' => $randomTwitterGenerator]);

        $resultOtherCount = DB::table('contacts')
            ->where('type', '!=', 'twitter')
            ->where('type', '!=', 'email')
            ->where('type', '!=', 'phone')
    ...
    protected function overwritePostAuthors()
    {
        ...
        // @codingStandardsIgnoreLine
        $randomName = DB::raw("CONCAT(lpad(conv(floor(rand()*pow(26,8)), 10, 36), 8, 0),' ',lpad(conv(floor(rand()*pow(26,8)), 10, 36), 8, 0))");
