...
class ImportForms extends ImportFromV2Job
{
    ...
    protected function processExtraParams()
    {
        ...
        $importMappings = (new Collection($formMaps))->map(function ($m) {
            ...
            // Create mapping
            $formMapping = new V2\ImportMapping([
                'import_id' => $this->importId,
                'source_type' => 'form',
                'source_id' => $m->from->id,
                'dest_type' => 'form',
    ...
    public function handle(
        ...
        $this->mappingRepo = $container->make('Ushahidi\Modules\V2\Contracts\ImportMappingRepository');
    ...
    public function importForms(
        ...
        FormRepository $destRepo,
        FormAttributeRepository $formAttributeRepo,
        TagRepository $tagRepo,
        V2\Mappers\FormMapper $mapper
    ) {
        // Set up importer
    ...
    protected function createDefaultForm()
    {
        ...
        $mappingRepo->create(new V2\ImportMapping([
            'import_id' => $this->importId,
            'source_type' => 'form',
            'source_id' => 0,
            'dest_type' => 'forms',
    ...
    protected function createDefaultAttributes($v3_formId, $v3_stageId, $v2_formId)
    {
        ...
        foreach ($this->defaultAttributes as $attr) {
            ...
            // Create attribute
            $attrId = $attrRepo->create(new FormAttribute(
    ...
    public function importFields(
        V2\Contracts\ImportMappingRepository $mappingRepo,
        FormAttributeRepository $destRepo,
        V2\Mappers\FormFieldAttributeMapper $mapper
    ) {
        // Set up importer
        $importer = new V2\Importer(
            'form_field',
