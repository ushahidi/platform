 ...
 * @package    Ushahidi\Modules\V2
 * @copyright  2014 Ushahidi
 * @license    https://www.gnu.org/licenses/agpl-3.0.html GNU Affero General Public License Version 3 (AGPL3)
 */

namespace Ushahidi\Modules\V2\Console;
...
class ImportFromV2Command extends Command
{
    ...
    public function handle(
        ...
        if ($this->option('params')) {
            ...
            $this->extraParams = new V2\ManifestSchemas\ImportParameters();
        }
        ...
        $import = new V2\Import();
        $importId = $importRepo->create($import);
        // + extract and save needed settings
        $this->info('Initializing import');
        $this->dispatcher->dispatchNow(new V2\Jobs\SetupImport($importId, $dbConfig, $this->extraParams));

        // Collect all tables data
        $this->info('Copying raw data');
        $this->dispatcher->dispatchNow(new V2\Jobs\CopyRawTables($importId, $dbConfig));

        // Create default survey
        // $this->info('Create default survey');
        // $this->dispatcher->dispatchNow(new V2\Jobs\CreateDefaultSurvey($importId, $dbConfig));

        // Import categories
        $this->info('Importing categories');
        $this->dispatcher->dispatchNow(
            new V2\Jobs\ImportCategories($importId, $dbConfig, $this->extraParams)
        );

        // Import forms
        $this->info('Import forms to surveys');
        $this->dispatcher->dispatchNow(new V2\Jobs\ImportForms($importId, $dbConfig, $this->extraParams));

        // Import users
        $this->info('Importing users');
        $this->dispatcher->dispatchNow(new V2\Jobs\ImportUsers($importId, $dbConfig));

        // Import incidents to posts
        $this->info('Importing incidents to posts');
        $this->dispatcher->dispatchNow(new V2\Jobs\ImportIncidents($importId, $dbConfig));

        $this->info('Importing reporters to contacts');
        $this->dispatcher->dispatchNow(new V2\Jobs\ImportReporters($importId, $dbConfig));

        $this->info('Importing messages');
        $this->dispatcher->dispatchNow(new V2\Jobs\ImportMessages($importId, $dbConfig));
