...
use Ushahidi\App\V5\Events\PostCreatedEvent;
use Ushahidi\App\V5\Events\PostUpdatedEvent;
use Ushahidi\App\V5\Http\Resources\PostCollection;
use Ushahidi\App\V5\Http\Resources\PostResource;
use Ushahidi\App\V5\Models\Post\Post;
use Ushahidi\App\V5\Models\Post\PostStatus;
use Ushahidi\App\V5\Exceptions\V5Exception;
use Illuminate\Support\Facades\DB;
use Ushahidi\App\V5\Common\ValidatorRunner;
use Ushahidi\App\V5\Models\Lock;
    ...
    protected function savePostValues(Post $post, array $post_content, int $post_id)
    {
        ...
        foreach ($post_content as $stage) {
            ...
            foreach ($stage['fields'] as $field) {
                ...
                $class_name = "Ushahidi\App\V5\Models\PostValues\Post" . ucfirst($type);
                if (!class_exists($class_name) &&
                    in_array(
                        $class_name,
                        [
                            'Ushahidi\App\V5\Models\PostValues\PostTitle',
                            'Ushahidi\App\V5\Models\PostValues\PostDescription'
                        ]
                    )
                ) {
                    continue;
