	...
	\Ushahidi\Core\Usecase\SearchRepository
{
	...
	public function countTotalPending($form_id, $total_sent) {
		...
		$total_active_contacts = DB::query(Database::SELECT,
			"select count(contact_id) as total from targeted_survey_state where form_id=$form_id
			 and survey_status IN ('RECEIVED RESPONSE','PENDING RESPONSE')")
		...
		$total_attributes = DB::query(Database::SELECT,
			...
			->execute($this->db)->get('total');
		$total_sent_to_inactive = $total_attributes - DB::query(Database::SELECT,
		"SELECT SUM(results.counted) as total FROM (SELECT count(form_attributes.form_stage_id) as counted from targeted_survey_state
			...
			and targeted_survey_state.survey_status='INACTIVE') as interna
			ON interna.contact_id = targeted_survey_state.contact_id
			WHERE form_attributes.priority <= interna.priority
		...
		return ($total_active_contacts * $total_attributes) - $total_sent + $total_sent_to_inactive;
	}
