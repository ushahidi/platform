...
class Task_Ushahidi_Import2x extends Minion_Task {
	...
	protected function _execute(array $options)
	{
		...
		$response = $this->_request('oauth/token')
			->method(Request::POST)
			->body("grant_type=password&client_id={$oauth_client_id}&client_secret={$oauth_client_secret}&username={$dest_username}&password={$dest_password}&scope=api posts forms tags sets")
			->execute();
