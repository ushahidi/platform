...
Feature: Testing the Config API
    ...
    Scenario: Updating a Config
        ...
        And the "result.id" property equals "test"
        And the "result.testkey" property equals "i am a teapot?"
    ...
    Scenario: Adding to Config with PUT
        ...
        Then the response is JSON
        And the "result.id" property equals "test"
        And the "result.nothing" property equals "new test value"
        And the "result.json_object_test.number" property equals "1234"
        And the "result.json_object_test.array" property contains "a"
        Then the guzzle status code should be 200
